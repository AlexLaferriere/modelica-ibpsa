...
equation
  ...
  deltaTb = IBPSA.Fluid.HeatExchangers.GroundHeatExchangers.BaseClasses.LoadAggregation.tempSuperposition(
    ...
    der(Q1_1) = if (time-19699200)>(nu[1]/2) then (Tb.Q_flow-Q1_1)/((rCel[1]/2)*tStep) else Tb.Q_flow/((rCel[1]/2)*tStep);
    der(Q1_2) = if (time-19699200)>nu[1] then (Q1_1-Q1_2)/((rCel[1]/2)*tStep) elseif (time-19699200)>(nu[1]/2) then Q1_1/((rCel[1]/2)*tStep) else 0;

  for jj in 1:i_cst loop
    if jj==1 then
      der(Q_i[jj]) = if (time-19699200)<=nu[jj] then (der(Q1_1)+der(Q1_2))/2 else der(Q1_1);
      //der(Q_i[jj]) = if time>=nu[jj] then (Tb.Q_flow-Q_i[jj])/(rCel[jj]*tStep) else Tb.Q_flow/(rCel[jj]*tStep);
    else
      der(Q_i[jj]) = if (time-19699200)>nu[jj] then (Q_i[jj-1]-Q_i[jj])/(rCel[jj]*tStep) elseif (time-19699200)>nu[jj-1] then Q_i[jj-1]/(rCel[jj]*tStep) else 0;
    end if;
  end for;
